...
export default class RewardsBody extends Component {
  constructor(props) {
    super(props);
    this.state={
        rewardList: null,
    ...
    componentWillReceiveProps(nextProps){
      ...
      this.props.searchReward !== nextProps.searchReward ?this.searchReward(nextProps.searchReward, this.state.originalList):'';
    }
  componentDidMount() {
  ...
  openPreviewUpload = (data) => {
    let { listUploadRewarded } = this.state;
    this.state.rewardList.map((dataInner) => {
      listUploadRewarded.push(dataInner.content);
    })
    this.setState({
  ...
  renderRewarded = (data, index) => {
      return(
        <tr key={`rewards${index}`} onClick={() => this.openPreviewUpload(data.content)}>          
         <td className="txt-oflo col-client-15 text-left">{data.creatorName}</td>
         <td className="col-client-15 text-left">{data.challenge}</td>    
         <td className="col-client-15 text-left">{data.rewardType}</td>
         ...
         <td className="col-client-15 text-left">{data.couponCode}</td>
  ...
  render(){     
    ...
    return (  
      <div id="page-wrapper">         
       <Scrollbars autoHide style={{ height: window.innerHeight - 30 }} renderThumbVertical={({ style, ...props }) =>
                  <div {...props} style={{ ...style, backgroundColor: '#fff' }}/>
                  }>
        <div className="rewards-body">
          <div className="container-fluid">
            <div className="row">
              <div className="content-rewards" style={{ height: window.innerHeight - 40 + 'px'}}>
                <div className="col-lg-9 col-md-9 col-sm-12 col-xs-12 rewards-list">
                <div className="table-responsive">
                                <table className="table table-reward">
                                    <thead>
                                        <tr>
                                            <th className="col-client-15 text-left">Creator Name</th>
                                            <th className="col-client-15 text-left">Challenge</th>  
                                            <th className="col-client-15 text-left">Reward Type</th>                                          
                                            <th className="col-client-10 text-center">Cash</th> 
                                             <th className="col-client-15 text-left">Reference</th>
                                            <th className="col-client-15 text-left">Coupon</th>
                                            <th className="col-client-15 text-center">Payment Status</th>
                                        </tr>
                                    </thead>                                    
                                    <tbody style={{height: window.innerHeight - 90 + 'px'}}>     
                                      {!this.state.loadingUpload &&  !searching && (this.state.rewardList && this.state.rewardList.length > 0) ? this.state.rewardList.map(this.renderRewarded, this, ''): <tr><td>No Upload Rewarded.</td></tr>}
                                    </tbody>
                                </table>
          ...
          <PreviewUpload listUploads={ this.state.listUploadRewarded } disableEdit={true} closeModal={this.closeModalPreview} isAll={false} 
